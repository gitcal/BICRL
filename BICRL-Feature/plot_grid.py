...
def plot_grid(Nx, Ny, state_grid_map, kk, N_demos, constraints=None, rewards=None,  optimal_policy=None):
    """
    ...
    for i in range(len(rewards)): 
        ...
        cnstr = state_grid_map[i]
        data[Ny - 1 - cnstr[1], cnstr[0]] = rewards[i]  

  
    ax.axes.xaxis.set_ticks([])
    ax.axes.yaxis.set_ticks([])
    
    cmap = plt.cm.RdYlGn#mpl.cm.get_cmap("OrRd").copy()
    
   
    im = plt.imshow(data, interpolation='nearest', cmap=cmap, vmin = -12, vmax =12, alpha=0.75)#vmin=0.1,
