...
class BIRL:
    def __init__(self, env, demos, beta, env_orig, prior, num_cnstr=0, epsilon=0.0001):
    ...
    def calc_ll(self, hyp_reward, prop_cnstr):
        ...
        log_prior = np.sum(prop_cnstr) * np.log(self.prior) + np.sum(1-prop_cnstr) * np.log(1-self.prior)#0.0  #assume unimformative prior
        log_sum = log_prior
        ...
        # include a prior here
        # log_sum = log_sum - #bernoulli.rvs(0.01)
        return log_sum
    ...
    def generate_proposal_bern_constr_alternating(self, old_constr, old_rew, step_size, ind = 0, stdev = 1):
        ...
        if ind % 20 == 0:   
            new_rew = old_rew + stdev * np.random.randn() 
            index = None     
        else:
            index = np.random.randint(len(old_constr))
            new_constr[index] = 1 if old_constr[index]==0 else 0
    ...
    def initial_solution_bern_cnstr(self):
        ...
        for i in range(len(new_constr)):
            new_constr[i] = bernoulli.rvs(0.01)
    ...
    def run_mcmc_bern_constraint(self, samples, stepsize, rewards_fix, normalize=True):
        ...
        for i in range(len(cur_constr)):
                if cur_constr[i] == 1 and cur_sol[i]!=2:
                    cur_sol[i] = cur_rew


        
        map_constr = cur_constr
        cur_ll = self.calc_ll(cur_sol, cur_constr)  # log likelihood
        ...
        for i in range(num_samples):
            ...
            for ii in range(len(prop_constr)):
                if prop_constr[ii] == 1 and prop_sol[ii]!=2:
                    prop_sol[ii] = prop_rew

           
            print('prop_rew',prop_rew)
            # calculate likelihood ratio test
            prop_ll = self.calc_ll(prop_sol, prop_constr)
            print(prop_ll)
            if prop_ll > cur_ll:
            ...
            if (i)%3999 ==0 and i>3:                
                mean_constraints = np.mean(self.chain_cnstr[int(np.floor(i-0.6*i)):i,:], axis=0)
               
                plot_grid.plot_grid_mean_constr(self.env.disc_x + 1, self.env.disc_y + 1, self.env.num_state_grid_map, i, mean_constraints, mean_constraints)
                plot_grid.plot_grid(self.env.disc_x + 1, self.env.disc_y + 1, self.env.num_state_grid_map, i, map_sol)
