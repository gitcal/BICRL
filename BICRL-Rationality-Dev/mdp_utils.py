...
def calculate_q_values(env, V=None, epsilon=0.0001):
  ...
  """
    ...
    Q_values = np.zeros((n, env.num_actions)) # was np.zeros
    for s in range(n):
        # if s not in np.nonzero(env.constraints)[0]:
        for a in range(env.num_actions):
...
def generate_boltzman_demo(env, beta, start_state):
    ...
    """
    current_state = start_state
    max_traj = env.num_states // 1  #this should be sufficiently long, maybe too long...
...
def FP_and_FN_and_TP(constraints, map_constr):
    ...
    # TPR_temp = np.sum(map_constr[np.nonzero(constraints)])
    # FPR_temp = np.sum(map_constr[no_cnstr_ind])
    ...
    Precision = np.sum(map_constr[np.nonzero(constraints)])/(np.sum(map_constr[np.nonzero(constraints)])+np.sum(map_constr[no_cnstr_ind]))
