...
def discretize_traj(env, traj, delta, max_speed):
    ...
    # state_vec = np.array([np.argmin(dist_1), np.argmin(dist_2),
    #     np.argmin(dist_3), np.argmin(dist_4)])
    return disc_traj
...
def generate_weird_demo(env, start_state):
    ...
    # temp=[env.goal[0]-current_state[0],env.goal[1]-tt[i][1]-current_state[1]]
    # act = env.cart_to_polar(temp)
    # noisy_rational_trajectory.append((np.array([env.goal[0],env.goal[1]]),np.array(act)))
    # IPython.embed()
    return noisy_rational_trajectory
