...
class BIRL:
    ...
    def run_mcmc_bern_constraint(self, samples, stepsize, rewards_fix, normalize=True):
        ...
        for i in range(num_samples):
            ...
            if (i)%200 ==0 and i>3:
                # plot_grid.plot_grid(8, 6, self.env.num_state_grid_map, i, map_sol)
                mean_constraints = np.mean(self.chain_cnstr[int(np.floor(i-0.8*i)):i,:], axis=0)
                # IPython.embed()
